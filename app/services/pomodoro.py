...
class PomodoroService:
    ...
    async def start_session(self, discord_id: str, channel_id: str, text_channel_id: str, duration_mins: int = 25) -> Tuple[bool, str]:
        ...
        async with db.execute(
            "SELECT pomodoro_start_time FROM users WHERE discord_id = ?",
            (discord_id,)
        ) as cursor:
        ...
        await db.commit()
        logger.info(f"User {discord_id} started Pomodoro in voice channel {channel_id} (text channel: {text_channel_id}) for {duration_mins} mins.")
        return True, f"Focus session started! Stay in your voice channel for {duration_mins} minutes to earn rewards."
    ...
    async def complete_session(self, discord_id: str) -> Tuple[bool, str, Dict[str, Any]]:
        ...
        async with db.execute("SELECT focus_points, focus_fruits FROM users WHERE discord_id = ?", (discord_id,)) as cursor:
        ...
        return True, f"🌟 Congratulations! Focus session completed!\n💰 Earning: **+100 Focus Points (FP)** and **+1 Focus Fruit**!{pet_reward_msg}", rewards_data

    async def cancel_session(self, discord_id: str, penalize: bool = True) -> Tuple[bool, str]:
