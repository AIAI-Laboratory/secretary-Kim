...
class PomodoroService:
    ...
    async def get_active_session(self, discord_id: str, db: Optional[aiosqlite.Connection] = None) -> Optional[Dict[str, Any]]:
        """Retrieve the active Pomodoro session details for the user."""
        if db is None:
            local_db = await self.db_service.get_db()
            try:
                return await self._get_active_session_internal(local_db, discord_id)
            finally:
                await local_db.close()
        return await self._get_active_session_internal(db, discord_id)

    async def _get_active_session_internal(self, db: aiosqlite.Connection, discord_id: str) -> Optional[Dict[str, Any]]:
        async with db.execute(
    ...
    async def complete_session(
        ...
        db = await self.db_service.get_db()
        try:
            session = await self.get_active_session(discord_id, db=db)
            if not session:
