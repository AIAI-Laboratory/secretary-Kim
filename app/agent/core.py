...
class KimAgent:
    ...
    async def process(self, request: AgentRequest) -> AgentResponse:
        ...
        system_instruction = self.context_engine.build_system_instruction(
            skill_descriptions
        )

        try:
            logger.info(
                f"Agent is processing request: '{request.content}' with {len(tools)} registered tools."
            )
            ...
            if function_calls:
                ...
                logger.info(
                    f"Gemini proposed calling function: '{call.name}' with arguments: {call.args}"
                )
                ...
                result = await self.registry.dispatch(
                    call.name, args_dict, skill_context
                )
