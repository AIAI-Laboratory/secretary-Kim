...
class AgentRequest(BaseModel):
    ...
    channel_id: str = Field(description="ID of the chat channel where the request was sent")
    ...
    discord_interaction: Optional[Any] = Field(None, description="discord.Interaction object")
...
class SkillContext(BaseModel):
    ...
    server_members: Dict[str, str] = Field(default_factory=dict, description="Map of Discord ID -> Display Name")
    voice_channels: Dict[str, str] = Field(default_factory=dict, description="Map of Channel ID -> Voice Channel Name")
    current_time_info: str = Field("", description="Current time info in human-readable format")
...
class SkillResult(BaseModel):
    ...
    success: bool = Field(description="True if execution is successful, False if failed")
    message: str = Field(description="Text feedback message for the user")
    embed: Optional[Any] = Field(None, description="Accompanying Discord Embed object (if any)")
    view: Optional[Any] = Field(None, description="Accompanying Discord UI View (buttons, dropdowns, etc.) (if any)")
    needs_confirmation: bool = Field(False, description="True if this action requires user confirmation (approve/reject)")
    data: Optional[Dict[str, Any]] = Field(None, description="Additional raw data returned to Agent Core")
...
class AgentResponse(BaseModel):
    ...
    skill_used: Optional[str] = Field(None, description="Name of the skill that processed this request")
